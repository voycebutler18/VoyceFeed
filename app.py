...
def get_specific_analysis_prompt(scraped_data: dict, profile_url: str):
    ...
    prompt = f"""
    As a world-class social media strategist, perform a direct and brutally honest analysis of the following scraped data from the public Facebook profile: {profile_url}.
    ...
    **Your Task:**
    Based ONLY on the specific text provided above, deliver a non-generic, actionable critique. If the scraped data says it failed or is empty, you MUST state that you cannot analyze the content and cannot provide a strategy. Do not make up generic advice.

    ### Bio Analysis:
    Critique the provided bio. Is it effective? What specific words should be changed? Provide a rewritten example.

    ### Post Content Analysis:
    - What is the primary theme in these posts?
    - Which post is the WEAKEST and why? Be specific.
    - Which post is the STRONGEST and why? Be specific.

    ### Specific Viral Strategy:
    Based on their STRONGEST post, give 3 concrete, specific ideas for new posts that follow that successful theme.
    """
    return prompt
...
def analyze():
    """Receives a URL, triggers the scraping bot, gets an AI analysis, and returns the result."""
